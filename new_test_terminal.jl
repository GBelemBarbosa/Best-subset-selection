...
function main()
    ...
    pPred = plot(ns_range, Predhist, labels=names, xlabel=L"n", ylabel=L"\frac{\Vert Ax-b\Vert^2}{\Vert b\Vert^2}", left_margin=5mm, dpi=600)
    savefig(pPred, "pnPred-$(plotname)$(specifics).png")

    pSim = plot(ns_range, Simhist, labels=names, xlabel=L"n", ylabel=L"\frac{|Supp(x)\cap Supp(x^\dagger)|}{\max\{|Supp(x)|,k^\dagger\}}", left_margin=5mm, dpi=600)
    savefig(pSim, "pnSim-$(plotname)$(specifics).png")

    pInf = plot(ns_range, Infhist, labels=names, xlabel=L"n", ylabel=L"\Vert x-x^\dagger\Vert_\infty", dpi=600)
    savefig(pInf, "pnInf-$(plotname)$(specifics).png")

    pSUP = plot(ns_range, SUPhist, labels=names, xlabel=L"n", ylabel=L"\Vert x\Vert_0", dpi=600)
