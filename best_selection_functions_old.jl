        ...
        R_SOLVE_SCRIPT_REF[] = RCall.rparse(raw"""
            ...
            if (penalty_str == "L0L2") {
                if (use_warmstart_val) {
                    fit <- L0Learn.fit(X_train, y_train, penalty=penalty_str, lambdaGrid=list(c(lambda_scaled)), nGamma=1, gammaMax=l2_val, gammaMin=l2_val, maxSuppSize=maxSuppSize, intercept=FALSE, initialBeta=beta_init)
                } else {
                    fit <- L0Learn.fit(X_train, y_train, penalty=penalty_str, lambdaGrid=list(c(lambda_scaled)), nGamma=1, gammaMax=l2_val, gammaMin=l2_val, maxSuppSize=maxSuppSize, intercept=FALSE)
                }
            } else if (penalty_str == "L0L1") {
                if (use_warmstart_val) {
                    fit <- L0Learn.fit(X_train, y_train, penalty=penalty_str, lambdaGrid=list(c(lambda_scaled)), nGamma=1, gammaMax=l1_val, gammaMin=l1_val, maxSuppSize=maxSuppSize, intercept=FALSE, initialBeta=beta_init)
                } else {
                    fit <- L0Learn.fit(X_train, y_train, penalty=penalty_str, lambdaGrid=list(c(lambda_scaled)), nGamma=1, gammaMax=l1_val, gammaMin=l1_val, maxSuppSize=maxSuppSize, intercept=FALSE)
                }
            } else {
                if (use_warmstart_val) {
...
function NSPG_PGCCD(x⁰, funcs; γₖ=0.0, work_xᵏ=nothing, work_rᵏ=nothing, work_xᵏ⁻¹=nothing, work_∇f=nothing, kwargs...)
    x, k = NSPG(x⁰, funcs; γₖ=γₖ, kwargs...)
    x, k2 = PGCCD(x, funcs; work_xᵏ=work_xᵏ, work_rᵏ=work_rᵏ, work_xᵏ⁻¹=work_xᵏ⁻¹, work_∇f=work_∇f, kwargs...)
    return x, k + k2
end
...
function PSI1(xˡ, funcs; work_xˡ=nothing, work_sˡ=nothing, kwargs...)
    ...
    # Residual: rᵏ = y - X*β (since r(β) = X*β - y, we negate)
    rᵏ = -r(xˡ)

    for i in nonzero_indices
        # Modified residual: rᵢ = y - X*β + X[:,i]*β_i  (removing variable i's contribution)
        # This lets us evaluate: X[:,j]' * rᵢ = X[:,j]'(y - Xβ + X[:,i]*β_i)
        # which equals the old formula: r⃰[j] + XTX[i,j]*β_i
        # (L0Learn's CDL012Swaps.cpp uses the same approach: riX = (r + B[i]*X[:,i])' * X)
        BLAS.axpy!(xˡ[i], view(X, :, i), rᵏ)  # rᵏ += X[:,i]*β_i

        jₘₐₓ = 0
        v⃰ₘₐₓ = 0.0

        @inbounds for j in zero_indices
            corr_j = dot(view(X, :, j), rᵏ)
            v⃰ = proxl0(corr_j)
        ...
        # Restore rᵏ to original residual
        BLAS.axpy!(-xˡ[i], view(X, :, i), rᵏ)  # rᵏ -= X[:,i]*β_i
